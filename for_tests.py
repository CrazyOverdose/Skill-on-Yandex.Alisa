...
def handle_dialog(user_message, flag, user_storage):
    """Функция обработки самой игры
    Args:
        user_message: Запрос пользователя
        flag: Новая ли сессия
    ...
    try:
        ...
        if user_message in COMMANDS:
            response = 'Команды для совершения хода: бросить кубик, я хожу, го, давай играть, играть, начать \n Команды, чтобы посмотреть количеcтво денег на бирже: биржа, деньги на бирже, деньги с биржи, сколько денег на бирже \nКоманды,чтобы узнать, размер вкладов в банке Алисы и пользователя: банк, вклады, деньги в банке, сколько денег в банке \nКоманды, чтобы узнать клетку Алисы и пользователя: моя ячейка, где я, где, мое положение, на какой я ячейке, клетка, моя клетка, на какой я клетке, местоположение \nКоманды для просмотра правил игры: правила, как играть, описание, описание игры \n Команды, чтобы посмотреть недвижимость Алисы и пользователя: недвижимость, моя недвижимость, собственность, моё, мое, моя собственность \n Команды, чтобы начать новую игру: новая игра, выход, начать новую игру \n Команды для просмотра количества сбережений Алисы и пользователя: деньги, мои деньги, сколько у меня денег, сколько денег, финансы, сбережения \n Команды, чтобы увидеть весь список клеток поля: ячейки, карта, поле, описание ячеек'
            return response, user_storage
        ...
        if user_storage["choice"]:
            ...
            if user_message in BURSEtake:
                ...
                response = 'Вы взяли ' + str(user_storage["exchange"] * 1.5) + ' $ с биржи'
            ...
            if user_message in BURSEgive:
                ...
                response = 'Вы оставили 100$ на бирже'

            if user_message not in BURSEgive + BURSEtake:
                response = 'Иногда ничего не делать - лучшее решение'
        ...
        if user_storage["prison1"] and user_storage["prison2"]:
            ...
            response = 'Алиса помогла сбежать вам из тюрьмы, ходите '
            return response, user_storage
        ...
        if user_storage["property"] != 0:
            if str(user_message) in BUY:
                if user_storage["moneyU"] < game.price_field[user_storage["field_cellU"]]:
                    response = 'У вас недостаточно средств '
                else:
                    ...
                    response = 'Поздравляю с приобретением! '
            else:
                response = 'Может, это действительно не лучшее вложение денег'
            ...
            if user_storage["moneyA"] <= 0:
                ...
                for i in range(1, 41):
                    if user_storage["propertyA"][i] == 1:
                        ...
                        break
                if flag != 0:
                    response = 'Алиса продала свою недвижимость ' + game.fields[i] + ' за ' + str(abs(game.price_field[i] // 2)) + ' $'
            ...
            if user_storage["moneyU"] <= 0:
                ...
                if flag != 0:
                    if not user_storage["crash"]:
                        response = 'Вы находитесь на грани банкротства, скажите, какую недвижимость хотите продать? '
                        ...
                        if user_message.isdigit():
                            if int(user_message) in game.havings:
                                if user_storage["propertyU"][(int(user_message))] == 1:
                                    response = 'Вы продали свою недвижимость ' + game.fields[int(user_message)] + ' за ' + str(abs(game.price_field[int(user_message)] // 2)) + ' $'
                                    ...
                                    response = 'Вы не владеете данным имуществом'
                                    return response, user_storage
                            else:
                                response = 'Зачем вы ввели номер несуществующей ячейки недвижимости?'
                                return response, user_storage
                        else:
                            response = 'Вы не ввели номер ячейки недвижимости'
                            return response, user_storage
        ...
        if user_message in WORDS:
            ...
            if not user_storage["users_turn"]:
                ...
                if user_storage["field_cellA"] == 36:
                    response = 'Ход Алисы \n' + game.fields[user_storage["field_cellA"]] + '\n Алиса попала на ' + game.fields[user_storage["field_cellA"] + 3]
                ...
                if int(user_storage["field_cellA"]) in game.havings:
                    ...
                    if user_storage["propertyU"][user_storage["field_cellA"]] == 1:
                        response = 'Ход Алисы \n' + 'Алиса попала на ваш участок: ' + game.fields[user_storage["field_cellA"]] + 'и заплатила ' + str(abs(game.price_foreign_field[user_storage["field_cellA"]] * k)) + '$\n' + '\n Если скуплены все ячейки недвижимости, стоимость попадания на них возрастает в 3 раза'
                    ...
                    if user_storage["propertyA"][user_storage["field_cellA"]] == 1:
                        response = 'Ход Алисы \n' + 'Алиса попала на свою территорию: ' + game.fields[user_storage["field_cellA"]]

                    if user_storage["propertyA"][user_storage["field_cellA"]] == 0 and user_storage["propertyU"][user_storage["field_cellA"]] == 0:
                        if b == 1:
                            response = 'Ход Алисы \n' + game.fields[user_storage["field_cellA"]] + ' и решила купить \n Если вы попадете на данный сектор, то заплатите ' + str(abs(game.price_foreign_field[user_storage["field_cellA"]])) + ' $' + '\n Если скуплены все ячейки недвижимости, стоимость попадания на них возрастает в 3 раза'
                ...
                if user_storage["field_cellA"] == 37:
                    ...
                    if y == 1:
                        ...
                        response = 'Ход Алисы \n' + game.fields[user_storage["field_cellA"]] + '\nАлиса оставила ' + str(money) + ' $ в банке'

                    if y == 3:
                        user_storage["moneyA"] = user_storage["moneyA"] + user_storage["bankA"]
                        response = 'Ход Алисы \n' + game.fields[user_storage["field_cellA"]] + '\nАлиса обналичила свой счёт в банке на сумму ' + str(user_storage["bankA"]) + ' $ '
            ...
            if user_storage["users_turn"]:
                ...
                if user_storage["field_cellU"] == 5:
                    response = 'Ваш ход \n' + game.fields[user_storage["field_cellU"]] + '\n Вы попали на ' + game.fields[user_storage["field_cellU"] - 2]
                ...
                if user_storage["field_cellU"] == 36:
                    response = 'Ваш ход \n' + game.fields[user_storage["field_cellU"]] + '\n Вы попали на ' + game.fields[user_storage["field_cellU"] + 3]
                ...
                if user_storage["field_cellU"] in game.havings:
                    ...
                    if user_storage["propertyA"][user_storage["field_cellU"]] == 1:
                        response = 'Ваш ход \n' + game.fields[user_storage["field_cellU"]] + ' \n Если скуплены все ячейки недвижимости, стоимость попадания на них возрастает в 3 раза \n Вы попали на недвижимость Алисы и заплатили ' + str(abs(game.price_foreign_field[user_storage["field_cellU"]] * k)) + '$\n'
                    ...
                    if user_storage["propertyU"][user_storage["field_cellU"]] == 0 and user_storage["propertyA"][user_storage["field_cellU"]] == 0:
                        response = 'Ваш ход \n' + game.fields[user_storage["field_cellU"]] + '\n Если Алиса попадет на эту недвижимость, она заплатит ' + str(abs(game.price_foreign_field[user_storage["field_cellU"]])) + ' $  \n Если скуплены все ячейки недвижимости, стоимость попадания на них возрастает в 3 раза\n Чтобы приобрести: введите "купить"'
        ...
        ## Если пользователь ввел какой-то неизвестный запрос
            ...
            response = "Простите, но я вас не поняла."
...
def end(text):
    ...
    response = text + 'Давай напомню правила: Каждый участник бросает кубик и в зависимости от выпавшего количества очков перемещает фишку по полю. За каждый пройденный круг банк выплачивает 200$. Ваша цель – не обанкротиться. \n Если вы остановились на поле недвижимости и оно не куплено другими участниками, у вас есть право на его покупку или отказ от покупки. \n Владение зданием дает право взыскивать арендную плату с человека, остановившегося на этом поле.\n \n Если вам не хватает средств на какие-то обязательные выплаты, вы становитесь банкротом. \n \n Чтобы посмотреть все доступные команды: "команды" \n Чтобы начать игру: "бросить кубик"'
